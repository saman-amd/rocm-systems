...
GPU target. Instrumentation rejects cross-variant insertion until an explicit
legalization path can prove that every copied instruction is legal for the
destination target.
