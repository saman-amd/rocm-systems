...
DecodeResult decodeVFmacLegacyF32Vop2(const MachineInst *opcode,
  ...
  if (reinterpret_cast<const Vop2::OpEncoding *>(inst)->src0 == amdgpu::SRC_DPP ||
    ...
    return emit_error.emit() << "V_FMAC_LEGACY_F32 does not support DPP";
  if (reinterpret_cast<const Vop2::OpEncoding *>(inst)->src0 == amdgpu::SRC_SDWA) [[unlikely]]
    return emit_error.emit() << "V_FMAC_LEGACY_F32 does not support SDWA";
...
DecodeResult decodeVCvtPkrtzF16F32Vop2(const MachineInst *opcode,
  ...
  if (reinterpret_cast<const Vop2::OpEncoding *>(inst)->src0 == amdgpu::SRC_DPP) {
    auto *dp = reinterpret_cast<const Vop2VopDpp16MachineInst *>(inst);
    if (!amdgpu::dpp::dpp_ctrl_is_valid(dp->dpp_ctrl, true, true, true)) [[unlikely]]
      return emit_error.emit() << "v_cvt_pkrtz_f16_f32: invalid DPP control";
  }
