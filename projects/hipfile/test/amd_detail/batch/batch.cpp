...
TEST(HipFileBatchDeadline, ZeroTimeoutIsAnImmediateDeadline)
{
    ...
    const auto deadline = makeBatchDeadline(&timeout);
...
TEST(HipFileBatchDeadline, PositiveTimeoutIsOffsetFromNow)
{
    ...
    const auto deadline = makeBatchDeadline(&timeout);
    const auto after    = std::chrono::steady_clock::now();
    const auto offset   = std::chrono::milliseconds{1500};

    ASSERT_TRUE(deadline.has_value());
    ASSERT_GE(*deadline, before + offset);
    ASSERT_LE(*deadline, after + offset);
}
