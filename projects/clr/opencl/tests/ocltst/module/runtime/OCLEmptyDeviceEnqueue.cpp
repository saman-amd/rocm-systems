...
// Sub-test 0: plain kernel with out-of-line helper, no enqueue (faithful BurnIn-style):
//             carries the device-enqueue hidden args at -O0 but never enqueues, and runs
//             with no default device queue.
// Sub-test 1: real device enqueue (regression guard, run with a device queue).
...
static const char* kernelName[] = {"empty_enqueue_plain", "real_enqueue"};
static const char* buildOpts[] = {"-cl-std=CL2.0 -cl-opt-disable", "-cl-std=CL2.0"};
...
void OCLEmptyDeviceEnqueue::open(unsigned int test, char* units, double& conversion,
  ...
  // Strict precondition for the no-queue O0 sub-test: the kernel must actually
  ...
  // Only the regression sub-test creates a default on-device queue. Sub-test 0
  // intentionally leaves deviceQueue_ == NULL (no default device queue).
  if (test == 1) {
...
void OCLEmptyDeviceEnqueue::run(void) {
  ...
  // The key assertion: dispatch must succeed and produce correct results even
  // with no default device queue for sub-test 0. Local size NULL lets the
  // runtime choose.
  ...
  // Read back the results only AFTER the kernel has completed. Mapping the
  // buffer before the dispatch and reading the stale pointer afterwards is
  // undefined behavior (a buffer must not be used by a kernel while mapped) and
  // is not guaranteed coherent on all ASICs.
  std::vector<cl_uint> host(TotalElements);
  error_ = _wrapper->clEnqueueReadBuffer(cmdQueues_[_deviceId], buffer, CL_TRUE, 0,
                                         TotalElements * sizeof(cl_uint), host.data(), 0, NULL,
                                         NULL);
  CHECK_RESULT((error_ != CL_SUCCESS), "clEnqueueReadBuffer() failed");

  for (cl_uint i = 0; i < TotalElements; ++i) {
    // Sub-test 0 (plain): res[i] = helper(i) = i*7-21.
    // Sub-test 1 (real enqueue): the child kernel overwrites res[i] = 0.
    cl_uint expected = (testID_ == 0) ? (cl_uint)(i * 7u - 21u) : 0u;
    if (host[i] != expected) {
