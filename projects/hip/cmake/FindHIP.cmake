...
message(DEPRECATION
    "FindHIP.cmake (MODULE mode) is deprecated and will be removed in a future release. "
    "Migrate to CMake-native HIP support: use find_package(hip CONFIG) and "
    "enable_language(HIP) instead. "
...
# For HIP_CXX_COMPILER as *hipcc derive the clang path from the hipcc location
# (hipcc lives in bin/, clang++ lives in lib/llvm/bin/ relative to the HIP root).
...
if(HIP_CXX_COMPILER MATCHES ".*hipcc")
  ...
  # hipcc is in <prefix>/bin; amdclang++/clang++ is in <prefix>/lib/llvm/bin
  get_filename_component(_HIP_ROOT_FROM_HIPCC "${HIPCC_PATH}/.." ABSOLUTE)
  set(_HIP_CLANG_CANDIDATE "${_HIP_ROOT_FROM_HIPCC}/lib/llvm/bin")
  if(EXISTS "${_HIP_CLANG_CANDIDATE}/clang++")
    set(HIP_CLANG_PATH "${_HIP_CLANG_CANDIDATE}")
  endif()
...
if(NOT APPLE)
    ...
    if(NOT HIP_ROOT_DIR)
        # Search in user specified path first; use share/hip/version as the marker
        # since hipconfig is deprecated and may not be present.
        find_path(
            HIP_ROOT_DIR
            NAMES share/hip/version
            PATHS
            "$ENV{ROCM_PATH}"
            "$ENV{ROCM_PATH}/hip"
            ENV HIP_PATH
    ...
    # Derive HIP_VERSION from share/hip/version (no hipconfig needed)
    if(NOT HIP_VERSION)
        set(_hip_version_file "${HIP_ROOT_DIR}/share/hip/version")
        if(EXISTS "${_hip_version_file}")
            file(STRINGS "${_hip_version_file}" _hip_version_lines)
            foreach(_line ${_hip_version_lines})
                if(_line MATCHES "^HIP_VERSION_MAJOR=([0-9]+)")
                    set(_hip_version_major "${CMAKE_MATCH_1}")
                elseif(_line MATCHES "^HIP_VERSION_MINOR=([0-9]+)")
                    set(_hip_version_minor "${CMAKE_MATCH_1}")
                elseif(_line MATCHES "^HIP_VERSION_PATCH=([0-9]+)")
                    set(_hip_version_patch "${CMAKE_MATCH_1}")
                endif()
            endforeach()
            if(_hip_version_major)
                set(HIP_VERSION "${_hip_version_major}.${_hip_version_minor}.${_hip_version_patch}"
                    CACHE STRING "Version of HIP as read from share/hip/version")
    ...
    # Derive HIP_PLATFORM without hipconfig: check env var, then detect from
    # compiler presence, defaulting to "amd" (nvidia support via CUDA is rare).
    if(NOT HIP_PLATFORM)
        if(DEFINED ENV{HIP_PLATFORM})
            set(HIP_PLATFORM "$ENV{HIP_PLATFORM}" CACHE STRING "HIP platform")
        elseif(EXISTS "${HIP_ROOT_DIR}/bin/nvcc" OR EXISTS "${HIP_ROOT_DIR}/bin/nvcc.exe")
            set(HIP_PLATFORM "nvidia" CACHE STRING "HIP platform")
        else()
    ...
    # Derive HIP_COMPILER without hipconfig: check env var, then derive from platform.
    if(NOT HIP_COMPILER)
        if(DEFINED ENV{HIP_COMPILER})
    ...
    # Derive HIP_RUNTIME without hipconfig: check env var, then derive from platform.
    if(NOT HIP_RUNTIME)
        if(DEFINED ENV{HIP_RUNTIME})
...
elseif("${HIP_COMPILER}" STREQUAL "clang")
    ...
    if(WIN32)
        ...
        # On Linux, use cmake -E env to pass HIP_CLANG_PATH (same approach as Windows)
        set(CMAKE_HIP_CREATE_SHARED_LIBRARY "${CMAKE_COMMAND} -E env HIP_CLANG_PATH=${HIP_CLANG_PATH} ${HIP_HIPCC_EXECUTABLE} ${HIP_CLANG_PARALLEL_BUILD_LINK_OPTIONS} <CMAKE_SHARED_LIBRARY_CXX_FLAGS> <LANGUAGE_COMPILE_FLAGS> <LINK_FLAGS> <CMAKE_SHARED_LIBRARY_CREATE_CXX_FLAGS> <SONAME_FLAG><TARGET_SONAME> -o <TARGET> <OBJECTS> <LINK_LIBRARIES>")
        set(CMAKE_HIP_CREATE_SHARED_MODULE "${CMAKE_COMMAND} -E env HIP_CLANG_PATH=${HIP_CLANG_PATH} ${HIP_HIPCC_EXECUTABLE} ${HIP_CLANG_PARALLEL_BUILD_LINK_OPTIONS} <CMAKE_CXX_LINK_FLAGS> <LINK_FLAGS> <OBJECTS> <SONAME_FLAG><TARGET_SONAME> -o <TARGET> <LINK_LIBRARIES> -shared" )
        set(CMAKE_HIP_LINK_EXECUTABLE "${CMAKE_COMMAND} -E env HIP_CLANG_PATH=${HIP_CLANG_PATH} ${HIP_HIPCC_EXECUTABLE} ${HIP_CLANG_PARALLEL_BUILD_LINK_OPTIONS} <FLAGS> <CMAKE_CXX_LINK_FLAGS> <LINK_FLAGS> <OBJECTS> -o <TARGET> <LINK_LIBRARIES>")
    endif()
